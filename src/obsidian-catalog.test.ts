...
describe("obsidian catalog pathing", () => {
  test("builds date-based note path", () => {
    ...
    expect(relativePath).toMatch(
      /^Research[\\/]Sandcode[\\/]\d{4}[\\/]\d{2}[\\/]\d{2}-01-owner-repo\.md$/,
    );
  });
  ...
  test("builds repo-based note path", () => {
    ...
    expect(relativePath).toMatch(
      /^Research[\\/]Sandcode[\\/]owner-repo[\\/]\d{4}-\d{2}-\d{2}-01-owner-repo\.md$/,
    );
  });

  test("keeps resolved note path inside vault", () => {
    const notePath = __testables.resolveNotePathWithinVault("/vault", "Research/Sandcode/note.md");
    expect(notePath).toBe(path.resolve("/vault", "Research/Sandcode/note.md"));
  });
